class TrackersController < ApplicationController
  ...
  def search
    ...
    @track_filter = Tracker.select("wb_id, part_code, employee_id, shift, device_id, count, target, to_char(created_at,'yyyy-mm-dd HH12:MI:SS:MS') as datetime").where("to_char(created_at,'yyyy-mm-dd') >= ? and to_char(created_at,'yyyy-mm-dd') <= ?", params[:created_at],params[:updated_at])
